...
struct hitsuki46_time_sync_state {
    bool synced;
...
static struct hitsuki46_time_sync_state state;
...
static int current_local_parts(const struct hitsuki46_time_sync_state *snapshot, int *year,
...
static unsigned int display_hour(unsigned int hour, enum hitsuki46_raw_hid_clock_mode clock_mode) {
...
bool hitsuki46_raw_hid_time_sync_format(char *buf, size_t len) {
    struct hitsuki46_time_sync_state snapshot;
    int year;
    unsigned int month;
    unsigned int day;
    unsigned int hour;
...
bool hitsuki46_raw_hid_time_sync_wants_seconds(void) {
    ...
    wants_seconds = state.synced && state.format_hint == HITSUKI46_RAW_HID_TIME_FORMAT_TIME_HMS;
    k_mutex_unlock(&state_lock);
