...
static struct hitsuki46_ai_usage_provider providers[AI_USAGE_SLOT_COUNT];
static K_MUTEX_DEFINE(providers_lock);

static int slot_for_provider(uint8_t provider) {
    switch (provider) {
    case HITSUKI46_RAW_HID_AI_PROVIDER_CODEX:
        return 0;
    case HITSUKI46_RAW_HID_AI_PROVIDER_CLAUDE_CODE:
        return 1;
    default:
        return -1;
    }
...
void hitsuki46_raw_hid_ai_usage_handle(const struct hitsuki46_raw_hid_packet *packet) {
    ...
    providers[slot] = (struct hitsuki46_ai_usage_provider){
        .present = true,
